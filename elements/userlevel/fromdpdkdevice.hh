...
public:
    ...
    static bool enable_interrupt(Task* t, void* data);
void do_sleep(int t);
protected:
...
#define CIO            100 //per packet cycles count
#define CCALL          43
#define CV             40 //single empty polling
#define CLIFF          0.8
...
#define MAX_ARRIVAL_RATE 14881*2

    class FDState { public:
        FDState() : mustresched(0),time_sleep(0) {};
        int mustresched;
        int time_sleep;

        uint64_t nb_rx_processed;
        uint64_t nb_idle_looped;
        struct rte_timer  power_timer;
        int recv = 0;
        int mini_period = MAX_ARRIVAL_RATE;    /* 0.1 percent of line rate, adjusting according to the real traffic and will be set to 1/10 of real traffic arrival rate*/
...
// If you are looking for SLEEP flags, they are in dpdkdevice.hh :)
...
static void
power_timer_cb(struct rte_timer *tim,
               void *arg);
